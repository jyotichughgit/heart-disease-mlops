...
def load_data(path: str) -> pd.DataFrame:
    ...
    # Note: download_data.py fills numerical with median, categorical with mode
    # SimpleImputer in the pipeline acts as a safety net for any remaining NaNs
    ...
    if len(missing_cols) > 0:
        ...
        logger.info("No missing values in dataset (filled by download_data.py)")

    logger.info(f"Data shape: {df.shape} | Target: {df[TARGET].value_counts().to_dict()}")
    return df
